...
const AddExpense = () => {
    ...
    useEffect(() => {
        if (expenses.length === 0) {
            fetch(`https://expense-tracker-5c73a-default-rtdb.firebaseio.com/${email}-expenses.json`)
    ...
    async function submitHandler(e) {
        ...
        if (patchID === '') {
            try {
                const response = await fetch(`https://expense-tracker-5c73a-default-rtdb.firebaseio.com/${email}-expenses.json`, {
                    method: 'POST',
            ...
            try {
                let url = `https://expense-tracker-5c73a-default-rtdb.firebaseio.com/${email}-expenses/${patchID}.json`;
    ...
    async function deleteHandler(e) {
        ...
        try {
            ...
            let url = `https://expense-tracker-5c73a-default-rtdb.firebaseio.com/${email}-expenses/${reqID}.json`;
        ...
        <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
            {totalObj.grandTotal>=10000&&<div style={{textAlign:'center'}}><Button style={{marginTop:'-65px'}} variant="primary" onClick={premiumHandler}>{!premiumStatus?'ACTIVATE PREMIUM':'PREMIUM ACTIVATED'}</Button></div>}
                ...
                <Container key={Math.random()} className={classes.form1}>
                    <div><h3>EXPENSES</h3><a href={downloadUrl} download="expenses.csv">{premiumStatus&&<img style={{ width: '30px',height: '30px',float:'right',marginTop:'-2.5rem' }} src='./download icon.png' alt='download icon'/>}</a></div>
